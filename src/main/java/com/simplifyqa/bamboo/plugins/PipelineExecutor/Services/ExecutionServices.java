...
public class ExecutionServices {
  ...
  private static final int REQUEST_TIMEOUT_IN_SECS = 300; // Added 5 minutes of Request Timeout for cloud containers

  private ExecutionImpl exec_obj = null;
  private static final String bannerPath =
    "src\\main\\resources\\io\\jenkins\\plugins\\SQA\\banner.txt";
  ...
  public static HttpResponse<String> getResponse(
    ...
    HttpRequest.Builder builder = HttpRequest
      ...
      .timeout(
        Duration.of(
          ExecutionServices.REQUEST_TIMEOUT_IN_SECS,
          ChronoUnit.SECONDS
        )
      );
    ...
    try {
      if (method.equalsIgnoreCase("GET")) request =
        builder.GET().version(HttpClient.Version.HTTP_2).build();

      if (method.equalsIgnoreCase("POST")) request =
        builder
  ...
  public boolean startExec() {
    ...
    try {
      ...
      while (
        !(
          Boolean.valueOf(
            ((JSONObject) new JSONParser().parse(response.body())).get(
                "success"
              )
              .toString()
          )
      ...
      switch (response.statusCode()) {
        case 200:
          ...
          this.exec_obj.setExecId(
              (long) (
                (JSONObject) new JSONParser().parse(response.body().toString())
              ).get("executionId")
            );
    ...
    try {
      ...
      switch (response.statusCode()) {
        ...
        case 400:
          ...
          this.printLog(
              ExecutionServices.getTimestamp() + this.exec_obj.getApp_url()
            );
          this.exec_obj.setExecStatus("FAILED");
          break;
        case 403:
